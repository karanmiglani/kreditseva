(function () {
  ...
  async function fetchLeads() {
    ...
    const tbody = document.getElementById('rawLeadsTbody');
    tbody.innerHTML = `<tr><td colspan="5" class="tbl-empty">Loading...</td></tr>`;
    if (table) { table.destroy(); table = null; }

    try {
      const resp = await fetch(`${window.location.origin}/api/dashboard/raw-leads?from=${from}&to=${to}`, {
        credentials: 'include'
      });
      ...
      if (data.success && data.leads?.length) {
        ...
        `).join('');
        table = new DataTable('#rawLeadsTable', { pageLength: 25, ordering: true, info: true });
      } else {
        tbody.innerHTML = `<tr><td colspan="5" class="tbl-empty">No raw leads found for selected date range.</td></tr>`;
      }
    } catch (err) {
  ...
  document.getElementById('exportBtn').addEventListener('click', () => {
    if (!table) return;
    const heads = ['ID','Mobile','Product','Source','Date'];
    const rows  = table.rows({ search: 'applied' }).data().toArray();
    const csv   = [heads.join(','), ...rows.map(r => Object.values(r).map(v => `"${v}"`).join(','))].join('\n');
    const a     = Object.assign(document.createElement('a'), {
      href: URL.createObjectURL(new Blob([csv], { type: 'text/csv' })),
      download: `raw-leads-${document.getElementById('dateFrom').value}.csv`
    });
    a.click();
  });
